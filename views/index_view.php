<?php require_once "header.php"; ?>

<h1 id="title">Liste des 20 meilleures personnes du mondeTEST</h1>


<?php if(!empty($message)): ?>
            <p class="big alert center-txt"> <?= Tools::showMessage($message) ?> </p>
<?php endif ?>

 
<section class="intro">                
    <article class="explanations-article">
            <p>
                Sur ce site, vous pouvez participer au classement des meilleures personnes de ce monde. "Meilleures" en ce sens qu'elles ont 
                contribué à améliorer notre monde que ce soit en sauvant des vies, en nous inspirant ou en permettant de mieux appréhender
                ce monde.
            </p>
            <p>
                Vous pouvez voter ou proposer une personne qui en fonction de ses actions ou de ses sacrifices a apporté une plus-value particulière à ce monde.
                Plus la contribution et le sacrifice sont importants, plus la personne aura de points de bases qui pourront ensuite être incrémentés par les votes.
            </p>
            <p>
                Si au contraire vous estimez qu'une personne ne devrait pas être dans le classement, vous pouvez cliquer sur l'option : contester.
                Ce choix ouvrira une boîte de dialogue vous permettant de justifier votre contestation.
            </p>
            <p>
                * C'est une question piège, il est possible d'atteindre le sommet tout en aidant les autres.
            </p>
    </article>
    <aside class="explanations-aside">
            <img class="explanations-img" src="assets/img/person/man_reading_m.png" alt="explications">
    </aside>
</section>

<main class="items-and-search-block">



        <section class="list-view">
                
                      
                        <?php foreach ($persons as $p): ?>
                                <article class="item-block" id="rank<?= $p->getRank() ?>">
                                       <div class="item-img-block">
                                            <div class="rank"> 
                                                    <?= $p->getRank(); ?> 
                                            </div>
                                             
                                             <a href="index.php?class=person&action=show&person=<?= $p->getId() ?>">
                                                <figure>
                                                    <img class="item-img" alt="<?= $p->getName() ?>" src="<?= $p->getImage() ?>">
                                                    <figcaption>
                                                            <?php $fullName = Tools::showIf($p->getAka()," | ").''.$p->getFirstname().' '.strtoupper($p->getName()); ?>   
                                                            <?= substr($fullName, 0, 70) ?> 
                                                    </figcaption>
                                                </figure>    
                                            </a>
                                            <!--
                                            <div>
                                                <progress value="10000" max="22000"></progress>
                                            </div>
                                            -->
                                           
                                            <div class="jauge">
                                                <div class="jauge-value" style="width: <?= Tools::jauge($p->getPoints()) ?>%;"></div> <!-- because progress and meter are not designed for this and width calculated -->
                                                <div class="jauge-txt">
                                                        <?= Tools::jauge($p->getPoints()) ?>%
                                                </div>
                                            </div>

                                           
                                        </div>

                                        <div class="item-txt-block">                                          
                                                
                                                <h3 class="margin_xs">    
                                                        <?= Tools::showIf($p->getAka()," | ").''.$p->getFirstname().' '.strtoupper($p->getName()); ?>
                                                </h3>
                                                <h4>    
                                                        <?= $p->getPoints().' points | '.$p->getGender(); ?>
                                                        <?= '| Profession(s) : '.$p->getProfession(); ?> 
                                                </h4>
												<p class="show-<?= $p->getId() ?>">
                                                    <?= substr(html_entity_decode($p->getDescription()), 0, 300); ?>
                                                </p>
                                                <p class="hide-<?= $p->getId() ?>">
                                                    <?= html_entity_decode($p->getDescription()); ?>
                                                </p> 
                                                                        
                                                <div class="contrib_n_risk">
                                                                                                                                 
                                                                <?php   
                                                                        $personContributions = new JnctPersonContribution(); 
                                                                        $personContributions = $personContributions->listByIdPerson($p->getId());
                                                                        $personContributions = Person::sortContributions($personContributions); 
                                                                ?>
                                                                
                                                                <ul>    
                                                                    <li>Contributions</li>
                                                                        <ul>
                                                                            <?php for($i =0; $i < count($personContributions); $i++): ?>
                                                                                    <li>                                                                       
                                                                                        <?= ' '.$personContributions[$i]['name']; ?>                                                    
                                                                                    </li>
                                                                            <?php endfor ?>
                                                                        </ul>
                                                                    
                                                                </ul>
                                                        

                                                          
                                                                <?php   $personRisks = new JnctPersonRisk(); 
                                                                        $personRisks = $personRisks->listByIdPerson($p->getId());
                                                                        $personRisks = Person::showRisksSorted($personRisks);                                                                                                          
                                                                ?>
                                                        
                                                                
                                                                <ul class="margin_xs">
                                                                    <li>Risques encourus</li>
                                                                        <ul>
                                                                            <?php for($i =0; $i < count($personRisks); $i++): ?>
                                                                                    <li>    
                                                                                    <?php //*'id:  '.$personContributions[$i]['id_contribution']; ?> 
                                                                                    <?= ' '.$personRisks[$i]['name']; ?> 
                                                                                    <?php //' '.$personContributions[$i]['points']; ?>                                                      
                                                                                    </li>
                                                                            <?php endfor ?>
                                                                        </ul>
                                                                </ul>
                                                        
                                                </div> <!-- contrib_n_risk -->   
                                                
                                                <p class="link-add">
                                                
                                                        <a href="index.php?class=person&action=add#title">
                                                            <i class="fas fa-user-plus"></i>                                                             
                                                            Ajouter une personne
                                                        </a>
                                                        <a href="#search">
                                                            <i class="fas fa-search"></i>
                                                        </a>
                                                </p>
                                        </div> <!-- Regular -->
                                        
                                        <nav class="nav-item-block">                                                        
                                        <?php if(!empty($_GET["page"]) && is_numeric($_GET["page"])): ?>
                                                    <?php $page = $_GET["page"]; ?>
                                                    <p>
                                                        <a id="vote<?= $p->getId() ?>" href="#">
                                                            <i class="far fa-thumbs-up"></i>voter pour <?= Person::showPronoun($p->getGender())?>
                                                        </a>
                                                    </p>
                                                    <p>                                                        
                                                        <a href="index.php?class=person&action=show&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>#title">
                                                            <i class="fas fa-info-circle"></i>voir fiche
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="index.php?class=person&action=edit&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>&page=<?= $page ?>#title">
                                                            <i class="far fa-edit"></i>modifier
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="views/user/test_increase.php?class=person&addDuplicates=1&person=<?= $p->getId() ?>#title">
                                                            <i class="fas fa-user-friends"></i>signaler doublon
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="index.php?class=person&action=showContest=<?= $p->getId() ?>#title">
                                                            <i class="far fa-thumbs-down"></i>contester
                                                        </a>
                                                    </p>
                                                    <p>                                                    
                                                        <a href="index.php?class=person&action=delete&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>&page=<?= $page ?>#title"><!--
                                                            --><i class="fas fa-user-slash"></i>supprimer
                                                        </a>
                                                    </p>
                                            <?php else: ?>
                                                    <p>
                                                        <a id="vote<?= $p->getId() ?>" href="#">
                                                            <i class="far fa-thumbs-up"></i>voter pour <?= Person::showPronoun($p->getGender())?>
                                                        </a>
                                                    </p>
                                                    <p>                                                        
                                                        <a href="index.php?class=person&action=show&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>#title">
                                                            <i class="fas fa-info-circle"></i>voir fiche
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="index.php?class=person&action=edit&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>#title">
                                                            <i class="far fa-edit"></i>modifier
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="views/user/test_increase.php?class=person&addDuplicates=1&person=<?= $p->getId() ?>">
                                                            <i class="fas fa-user-friends"></i>signaler doublon
                                                        </a>
                                                    </p>
                                                    <p>
                                                        <a href="index.php?class=person&action=showContest=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>#title">
                                                            <i class="far fa-thumbs-down"></i>contester
                                                        </a>
                                                    </p>
                                                    <p>                                                    
                                                        <a href="index.php?class=person&action=delete&person=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>#title"><!--
                                                            --><i class="fas fa-user-slash"></i>supprimer
                                                        </a>
                                                    </p>

                                            <?php endif ?>
                                                    <p>
                                                        <a href="index.php?class=user&action=testSession">Test session</a>
                                                    </p>
                                        </nav>                  
                                        
                                </article> <!-- article_one_aside -->
                                                                

                        <?php endforeach ?>
        </section>
        <aside class="search-block">     

            <form action="" method="post">
                    <div class="search-form">
                            <h3 id="search">Rechercher une personne</h3>  
                                    
                                            <label for="name">Nom</label>                                    
                                            <input type="text" id="name" name="name">

                                            <label for="firstname">Prénom</label>
                                            <input type="text" id="firstname" name="firstname">

                                            <label for="aka">Surnom</label>
                                            <input type="text" id="aka" name="aka">
                                                                                                             
                                    <button class="enhanced_button" type="submit">Rechercher</button>
                    </div>                                          
            </form>

            <?php if(isset($_POST["name"]) || isset($_POST["firstname"]) || isset($_POST["aka"])): ?>

                    <?php $seeked_person = Person::searchPersons(); ?>
                    <?php if($seeked_person != null): ?>
                            <h2>Votre recherche</h2>
                            <?php foreach($seeked_person as $sp): ?>
                                    
                                    <p class="search-p">                                                                                            
                                        <figure>
                                            <a href="index.php?class=person&action=show&person=<?= $sp->getId() ?>#title">
                                                <img class="search-img" src="<?=$sp->getImage() ?>" alt="<?= $sp->getAka().' - '.$sp->getFirstname().' '.$sp->getName() ?>"> <!-- Pas de figure car ces miniatures ne doivent pas apparaître ds les moteurs de recherche -->
                                                <figcaption>
                                                        <?= $sp->getAka().' ' ?>
                                                        <?= $sp->getFirstname().' '.$sp->getName() ?>
                                                        <?= $sp->getBirth(); ?>
                                                </figcaption>
                                            </a>
                                        </figure> 
                                    </p>   
                                                        
                            <?php endforeach ?>
                    <?php endif ?>
            <?php endif ?>

        </aside> <!--  <aside class="width_xs annex padding_m">    -->        
        
</main> <!--End of Main ([section[article] + aside]] + section) -->

<nav class="center-txt">
    <a class="enhanced_button-a" href="index.php?class=person&action=showList#title">voir classement complet</a>
</nav>



<?php require_once "footer.php"; ?>