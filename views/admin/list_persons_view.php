...
<h1 id="title" class="title-with-sub">Liste des meilleures personnes du monde ADMIN</h1>
                    ...
                    <?php else: ?>
                        ...
                        <p>
                            <a href="index.php?class=person&action=showContest=<?= $p->getId() ?>&rank=<?= $p->getRank() ?>">
                                <i class="far fa-thumbs-down"></i>contestations
                            </a>
                        </p>
                        <p>                                                    
    ...
    <?php for($page = 0; $page < count($pagination); $page++):  ?> <!-- key 0 contains the range for pagination -->
        <?php if($page > 0 && !empty($_GET["page"]) && is_numeric($_GET["page"])): ?>
            <?php if($page == $_GET["page"]): ?>
                <a class="page-selected" href="index.php?class=person&action=showList&page=<?= $page ?>&offset=<?= $pagination[$page]; ?>&limit=<?= $pagination[0]; ?>#title">
                    <?= $page ?>
                </a>
            <?php else: ?>
                <a class="page" href="index.php?class=person&action=showList&page=<?= $page ?>&offset=<?= $pagination[$page]; ?>&limit=<?= $pagination[0]; ?>#title">
                        <?= $page ?>
                </a>
            <?php endif ?> 
        <?php else: ?>                    
